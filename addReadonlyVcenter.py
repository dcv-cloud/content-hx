...
def addReadonlyUserToVcenter():

    logging.info('About to add readonly user to vcenter')
    ...
    myPermission.entity=root
    myPermission.principal = 'DCLOUD\\readonly' #'DCV\\v561user1'
    ...
    try:
        content.authorizationManager.SetEntityPermissions(root,myPermissions)
        logging.info('Permissions assgined to vcenter ' + str(root)) 
        return True
    except Exception as e:
        logging.info("Error while assigning permissions to  vcenter "+ str(root) +"   : " + str(e))
        return False



#mountAllVmfs()
addReadonlyUserToVcenter()
